...
import br.com.plusoftomni.integration.domain.telephonyplatform.event.GenericEvent;
...
@EventHandler
@Profile("example")
public class OmniCTIGenericEventHandler {
    ...
    @Handle(CTIEvents.GENERICEVENT)
    public void execute(GenericEvent event){
        event.getArguments();

        callbackDispatcher.dispatch(new CTIResponse("status", 0, "genericEvent called.", Collections.unmodifiableMap(Stream.of(
                new AbstractMap.SimpleEntry<>("arg1", "one"),
                new AbstractMap.SimpleEntry<>("arg2", "two"))
                .collect(Collectors.toMap((e) -> e.getKey(), (e) -> e.getValue())))));

        logger.trace("GENERICEVENT Event Called");
    }
}
